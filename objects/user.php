...
class User {
    ...
    static function getAllUsersByStreaming() {
        //will receive
        //current=1&rowCount=10&sort[sender]=asc&searchPhrase=
        global $global;
        $sql = "SELECT users.id,name,email,user, count(LiveLinks.users_id) + count(live_transmitions.users_id) as userCount from users inner join LiveLinks on users.id = LiveLinks.users_id inner join live_transmitions on users.id = live_transmitions.users_id group by users.id";
